...
import { Mail, ArrowRight, Loader2, CheckCircle2, UserPlus, Link2 } from "lucide-react";
...
function SignUp() {
      ...
      if (isSignInWithEmailLink(auth, window.location.href)) {
        ...
        if (emailForSignIn) {
          try {
            ...
            toast.success("Welcome to Scribbly! Account created successfully.");
  ...
  const handlePasswordlessSignUp = async () => {
    ...
    try {
      ...
      console.error("Error with passwordless sign-up:", error);
  ...
  const handleSocialSignUp = async (e, providerType) => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        toast.success("Welcome to Scribbly!");
      } else {
        await setDoc(userRef, {
          uid: user.uid,
          email: user.email,
    ...
    >
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Join Scribbly and discover premium stationery</p>
        </div>
          ...
          {!emailSent ? (
            <>
              ...
              <form onSubmit={handleEmailSignUp} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-3 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent transition duration-200 placeholder-gray-400"
                    required
                    disabled={loading || recaptchaChecking}
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading || recaptchaChecking || !email || !password || !confirmPassword}
                ...
                <div className="flex items-center justify-between text-sm pt-1">
                  ...
                  <Link to="/password-reset" className="text-blue-600 hover:text-blue-800 hover:underline transition-colors">
                    Forgot Password?
                  </Link>
                </div>
              </form>
              ...
              <div className="space-y-3">
                ...
                {/* Passwordless Sign Up — separated with its own divider */}
                <div className="relative pt-1">
                  <div className="absolute inset-0 flex items-center" style={{ top: '0.25rem' }}>
                    <div className="w-full border-t border-dashed border-gray-200"></div>
                  </div>
                  <div className="relative flex justify-center text-xs mb-3">
                    <span className="px-3 bg-white text-gray-400">prefer no password?</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={passwordlessLoading || recaptchaChecking || !email}
                  className="w-full bg-white border border-dashed border-gray-300 text-gray-500 py-3 px-4 rounded-lg font-medium hover:bg-gray-50 hover:border-gray-400 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 transition duration-200 flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {passwordlessLoading ? (
                    <>
                ...
                {!email && (
                  <p className="text-xs text-center text-gray-400">Enter your email above to use passwordless sign up</p>
                )}
              </div>
        ...
        {/* Privacy Notice */}
        <p className="mt-4 text-xs text-center text-gray-500">
          Protected by reCAPTCHA and subject to the Scribbly{" "}
          <a href="/privacy" className="underline hover:text-gray-700">Privacy Policy</a>
        </p>
      </div>
    </m.div>
