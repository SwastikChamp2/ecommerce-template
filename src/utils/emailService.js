...
const sendViaApiEndpoint = async (emailData) => {
  try {
    ...
    const formattedFrom = fromEmail.includes('<') ? fromEmail : `Scribbly <${fromEmail}>`;
...
const sendOrderConfirmationEmail = async (order, user) => {
  ...
  try {
    ...
    const emailData = {
      to: user.email,
      subject: `Scribbly - Order Confirmation #${order.orderId}`,
      body: emailBody,
    };
              ...
              <tr>
                <td style="padding: 32px 40px; border-bottom: 1px solid #e5e7eb;">
                  <table width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                      <td align="left">
                        <h1 style="margin: 0; font-size: 24px; font-weight: 600; color: #111827; letter-spacing: -0.5px;">Scribbly</h1>
              ...
              <tr>
                <td style="padding: 32px 40px; background-color: #f9fafb; border-top: 1px solid #e5e7eb; text-align: center;">
                  <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                      <td style="text-align: center;">
                        <p style="margin: 0 0 8px 0; font-size: 14px; font-weight: 600; color: #111827;">Scribbly</p>
...
const sendMagicLinkEmail = async (email, magicLink) => {
  ...
  try {
    ...
    const emailData = {
      to: email,
      subject: 'Sign in to Scribbly - Your Magic Link',
      body: emailBody,
    };
...
<head>
  ...
  <meta name="format-detection" content="telephone=no, date=no, address=no, email=no">
  <meta name="x-apple-disable-message-reformatting">
  <title>Sign in to Scribbly</title>
  <style type="text/css">
    body, table, td {
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Helvetica Neue', Arial, sans-serif !important;
    }
                  ...
                  <table width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                      <td style="text-align: center;">
                        ...
                        <p style="margin: 0 0 24px 0; color: #6b7280; font-size: 15px; line-height: 1.5;">Click the button below to securely sign in to your Scribbly account.</p>
                        ...
                        <div style="text-align: center; margin-bottom: 24px;">
                          <a href="${magicLink}" target="_blank" style="display: inline-block; background-color: #111827; color: #ffffff; font-size: 16px; font-weight: 500; text-decoration: none; padding: 14px 32px; border-radius: 6px; margin: 0 auto;">Sign In to Scribbly</a>
                        </div>
